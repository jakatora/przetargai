...
import { cpvAffinity } from './cpv.js';
import { matchKeywords, normalize } from './textNorm.js';
...
function regionPasuje(regionProfil, regionPozycja) {
  // Cloud Functions: profil trzyma `regiony` jako kody TERYT, a pozycja przychodzi
  // z kodem (TED → NUTS) albo nazwą. Porównujemy KODY, bo dwa formaty już raz
  // schowały całe źródło w filtrze katalogu (etap 4).
  if (Array.isArray(regionProfil)) {
    const kodPozycji = kodWojewodztwa(regionPozycja);
    if (!kodPozycji) return false;
    return regionProfil.some((r) => kodWojewodztwa(r) === kodPozycji);
  }
...
function ocenPozycje(pozycja, profil) {
  ...
  const regionProfilu = Array.isArray(profil.regiony) && profil.regiony.length ? profil.regiony : profil.region;
  const zgodnoscRegionu = regionPasuje(regionProfilu, regionZ(pozycja));
  ...
  if (zgodnoscRegionu) {
    const nazwa = Array.isArray(regionProfilu) ? nazwaWojewodztwa(regionZ(pozycja)) : String(profil.region).trim();
    powody.push(`Zgodny region: ${nazwa}.`);
  }
