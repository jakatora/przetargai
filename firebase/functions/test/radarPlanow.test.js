...
// ── Cloud Functions: profil trzyma regiony jako KODY TERYT (PATCH /auth/me), a pozycje
// z TED przychodza z kodem wyliczonym z NUTS. Dwa formaty (kod vs nazwa) juz raz
// schowaly cale zrodlo BK w filtrze katalogu — tu pilnujemy obu kierunkow.
test('profil.regiony (kody TERYT) trafia w region pozycji podany kodem lub nazwa', () => {
  const profil = { cpv: ['45233140'], regiony: ['14', '12'], dzisiaj: '2026-09-24' };
  const [kodem] = dopasujPozycjePlanu({
    pozycjePlanow: [{ przedmiot: 'Przebudowa drogi', cpv: '45233140', region: '14', terminWszczecia: '2026-11' }],
    profil,
  });
  assert.equal(kodem.uzasadnienie.zgodnoscRegionu, true);
  const [nazwa] = dopasujPozycjePlanu({
    pozycjePlanow: [{ przedmiot: 'Przebudowa drogi', cpv: '45233140', region: 'małopolskie', terminWszczecia: '2026-11' }],
    profil,
  });
  assert.equal(nazwa.uzasadnienie.zgodnoscRegionu, true, 'nazwa województwa = kod 12');
  const [obcy] = dopasujPozycjePlanu({
    pozycjePlanow: [{ przedmiot: 'Przebudowa drogi', cpv: '45233140', region: '02', terminWszczecia: '2026-11' }],
    profil,
  });
  assert.equal(obcy.uzasadnienie.zgodnoscRegionu, false);
  assert.equal(kodem.wynik - obcy.wynik, WAGA_REGION);
});

test('powod regionu podaje NAZWE wojewodztwa, nie goly kod', () => {
  const [w] = dopasujPozycjePlanu({
    pozycjePlanow: [{ przedmiot: 'x', cpv: '45233140', region: '14', terminWszczecia: '2026-11' }],
    profil: { cpv: ['45233140'], regiony: ['14'], dzisiaj: '2026-09-24' },
  });
  assert.ok(w.uzasadnienie.powody.some((p) => /mazowieckie/i.test(p)), w.uzasadnienie.powody.join(' | '));
});
